...
{
    if (_timerType == TimerLabelTypeTimer) {
        [self setCountDownTime:timeToAdd + timeUserValue];
    }else if (_timerType == TimerLabelTypeStopWatch) {
...
-(void)updateLabel{
    ...
    if ([_delegate respondsToSelector:@selector(timerLabel:customTextToDisplayAtTime:)]) {
        ...
        if(_shouldCountBeyondHHLimit) {
            ...
            int hours = (_timerType == TimerLabelTypeStopWatch)? ([self getTimeCounted] / 3600) : ([self getTimeRemaining] / 3600);
